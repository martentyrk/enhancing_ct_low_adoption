...
class NodeClsTrainer:
    def __init__(
        ...
        self.set_criterion = torch.nn.MSELoss()
        self.lr = params['lr']
    ...
    def train(self):
        ...
        if self.model_name in ['gcn', 'gcnmf']:        
            ...
            for features in self.train_loader:
                ...
                loss = self.criterion(pred, y.to(torch.float))
        ...
        with torch.no_grad():
            if self.model_name in ['gcn', 'gcnmf']:
                ...
                for features in loader:
                    X = features['features']
                    y = features['outcome'].to(torch.float)
                    X, y = X.to(device), y.to(device)
                    fn_pred = features['fn_pred'].to(device).detach()
                    output = model(X, fn_pred)
    ...
    def run(self):
        ...
        torch.save(self.model, "results/models/fullmodel_" + fname_model + '.pth')
